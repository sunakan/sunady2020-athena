...
data "aws_iam_policy_document" "athena_policy" {
  ...
  statement {
    ...
    resources = ["${var.log_bucket_arn}/athena-result/*"]
  }
}
resource "aws_iam_user" "athena_user" {
  name = "${var.service_name_with_env}-athena-user"
  path = "/app/"
  tags = var.common_tags
}
resource "aws_iam_user_policy" "attach-athena-user" {
  name   = "${var.service_name_with_env}-athena-user"
